...
void basic_model::draw2(const glm::mat4 &view, const glm::mat4 proj) {
...
void basic_model::Teapot(const glm::mat4 &view, const glm::mat4 proj, const std::vector<glm::vec3> &sliderPts, const float &start) {
//	mat4 modelview = view * modelTransform;
...
void basic_model::Coord1(const glm::mat4 &view, const glm::mat4 proj) {
    mat4 modelview = view * modelTransform;
    modelview = glm::scale(modelview, vec3(0.1));
...
void basic_model::Coord3(const glm::mat4 &view, const glm::mat4 proj, std::vector<glm::vec3> &controlPts, std::vector<glm::vec3> &interpolatedPts) {
    mat4 modelview = view * modelTransform;
...
void basic_model::Coord4(const glm::mat4 &view, const glm::mat4 proj, std::vector<glm::vec3> &controlPts, std::vector<glm::vec3> &interpolatedPts) {
    mat4 modelview = view * modelTransform;
    //modelview = glm::scale(modelview, vec3(0.2));
...
Application::Application(GLFWwindow *window) : m_window(window) {
    ...
    m_skeleton.m_joint_mesh.shader = shader;
    m_skeleton.m_bone_mesh.shader = shader;
    m_skeleton.m_arrowx_mesh.shader = shader;
    m_skeleton.m_arrowy_mesh.shader = shader;
    m_skeleton.m_arrowz_mesh.shader = shader;

    ///m_skeleton.m_joint_mesh.mesh = load_wavefront_data(CGRA_SRCDIR + std::string("/res//assets//teapot.obj")).build();
    m_skeleton.m_joint_mesh.color = vec3(0, 1, 1);
    m_skeleton.m_bone_mesh.color = vec3(1, 1, 1);
    m_skeleton.m_arrowx_mesh.color = vec3(1,0,0);
    m_skeleton.m_arrowy_mesh.color = vec3(0,1,0);
    m_skeleton.m_arrowz_mesh.color = vec3(0,0,1);

    /// <---- ASF Files - Completion ------>
    m_skeleton.shader = shader;

    m_skeleton.skel = skeleton_data("/Users/gray/Documents/UNI/CGRA350/A2 copy/Assignment 1/work/res/assets/priman.asf");
    skeleton_data skelo = skeleton_data("/Users/gray/Documents/UNI/CGRA350/A2 copy/Assignment 1/work/res/assets/priman.asf");
    //AMC Files - Challenge
    anim = skeleton_animation(CGRA_SRCDIR + std::string("/res/assets/walking_priman.amc"), skelo);

   /// <----- PART 2 - Animation ------------>
    m_sphere_red.shader = shader;
    m_sphere_red.color = vec3(1, 0, 0);

    m_sphere_green.shader = shader;
    m_sphere_green.color = vec3(0, 1, 0);

    m_sphere_blue.shader = shader;
    m_sphere_blue.color = vec3(0, 1, 1);
...
void Application::render() {
    ...
    if(animate) {
        m_skeleton.pose = anim.poses[animIndex];
        cout << anim.poses[animIndex].boneTransforms[0].rotation.x;

        if (animIndex < anim.poses.size()-1) {
            animIndex++;
            cout << "index: " << animIndex;
        } else {
            animIndex = 0;
        }
    }
    ...
    ///Draw the 5 control points of the Catmull-Rom Splice
    mat4 modelTrans(view);
    modelTrans = glm::translate(modelTrans, sliderPts[start]);
    m_model.Teapot(modelTrans, proj, sliderPts, start);
    m_sphere_green.Coord1(glm::translate(view, controlPts[1]), proj);
    m_sphere_blue.Coord1(glm::translate(view, controlPts[2]), proj);
    m_sphere_blue.Coord1(glm::translate(view, controlPts[3]), proj);
    m_sphere_blue.Coord1(glm::translate(view, controlPts[4]), proj);
    m_sphere_red.Coord1(glm::translate(view, controlPts[5]), proj);
...
//     m_sphere_r.draw1(glm::translate(view, camControlPts[5]), proj);
    ...
    //}
    m_spline.Coord3(view, proj, controlPts, interpolatedPoints);
    m_cam_spline.Coord4(view, proj, camControlPts, camInterpolatedPoints);
...
void Application::renderGUI() {
    ...
    int pose_index = 0;
    if (ImGui::Combo("Poses", &pose_index, "Priman\0Sitting\0Walking\0", 3)) {
        switch (pose_index) {
            case 0:
                ...
                break;
            case 1:
                m_skeleton.skel = skeleton_data("/Users/gray/Documents/UNI/CGRA350/A2 copy/Assignment 1/work/res/assets/sitting_pose.asf");
                break;
            case 2:
                m_skeleton.skel = skeleton_data("/Users/gray/Documents/UNI/CGRA350/A2 copy/Assignment 1/work/res/assets/walking_pose.asf");
                break;
        }
    }
